...
open_key=../key/publickey

#путь к файлу в котором храниться приватный ключ
privat_key=../key/privatekey
...
#создание подписи
openssl dgst -sha256 -sign ../key/privatekey -out signature "$destination_file" 

#проверка подписи(выведет ОК в консоль если проверка прошла успешно)
openssl dgst -sha256 -verify ../key/publickey -signature signature "$destination_file"
...
mkdir "$directory"/signature

#копируем публичный ключ в папку key 
cp ../key/publickey "$directory/key/"

#копируем подпись в папку  signature
cp signature "$directory/signature/"
